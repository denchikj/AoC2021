...
let processData
    ...
    |> Seq.iter
        (fun ((a, b), times) ->
            ...
            let ac =
                match tmp.TryGetValue((a, c)) with
                | (true, v) -> v
                | _ -> 0
            ...
            let cb =
                match tmp.TryGetValue((c, b)) with
                | (true, v) -> v
                | _ -> 0
            ...
            let ab =
                match tmp.TryGetValue((a, b)) with
                | (true, v) -> v
                | _ -> 0
            ...
            let cv =
                match tmpCnt.TryGetValue(c) with
                | (true, v) -> v
                | _ -> 0
    ...
    let elementCount =
        tmpCnt |> Seq.map (|KeyValue|) |> Map.ofSeq
...
let execute filename times =
    ...
    let cnt =
        ...
        |> (fun (_, cnt) -> cnt |> Map.toSeq |> Seq.map snd)

    let min = cnt |> Seq.min
    let max = cnt |> Seq.max
    max - min

let solve filename = execute filename 10

let solve2 filename = execute filename 40

...
let numbers =
    input.[0].Split ',' |> Seq.map int |> Seq.toArray
...
let boards =
    { 0 .. 99 } |> Seq.map getBoard |> Seq.toArray
...
let isWinner (draws: Set<int>) (board: int [] []) =
    ...
    let isMatch (line: int []) =
        line
        |> Array.filter (fun c -> not (draws.Contains c))
        |> Array.isEmpty

    let matchingLines =
        lines |> List.filter isMatch |> List.length
...
let getLosers (draw: Set<int>) =
    boards
    |> Seq.filter (fun board -> not (isWinner draw board))
    |> Seq.toList
